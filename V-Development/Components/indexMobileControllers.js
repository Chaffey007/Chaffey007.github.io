(function () {
    ...
    myApp.controller('logisticsControllerMobile',['$scope', '$rootScope', '$timeout', '$log', '$http', '$location', 'GlobeVars', 'scrollPageService', '$sce', 'currencyConvert', function($scope, $rootScope, $timeout, $log, $http, $location, GlobeVars, scrollPageService, $sce, currencyConvert){
        ...
        $scope.menuOptions = ['Home','About Us','Technology Overview','Contact Us','More'];
        $scope.menuOptionsSVG = ['./Images/SVG/neighborhood.svg','./Images/SVG/monitor.svg','./Images/SVG/innovation.svg','',''];
        for(let a = 0; a < $scope.menuOptions.length; a++){
            $scope.menuOptionList.push({
                id: a,
                title: $scope.menuOptions[a],
                svg: $scope.menuOptionsSVG[a]
            });
        }
        ...
        $scope.miniLogin = function() {
            ...
            $http({
                ...
                function(){
                    $scope.miniLogResp = "Unknown Error";
                });
        };
        ...
        function readyShipmentDetails(){
            ...
            if(searchProcessed[0].listType === 'Ship'){
                ...
                if($scope.quoteOfferList[0].car !== undefined){
                    readyPayment();
                }
            }
            $timeout(function(){
        ...
        function testForm(){
            if($scope.adrPage === 1){
                $scope.validStep = $scope.validStepOne = ((orStr !== '') && (orCity !== '') && (orProv !== '') && (orPost !== '') && (orCountry !== ''));
            }else{
                $scope.validStep = $scope.validStepOne = ((deCountry !== '') && (deStr !== '') && (deCity !== '') && (deProv !== '') && (dePost !== ''));
            }
        ...
        //... Move List on dot select ...
        $scope.selSpecList = function(index){
            $scope.actOffer = index;
            readyPayment();
            $scope.validStep = validStepFour = true;
        };
        ...
        //... Show iframe ...
        $scope.togFrame = TogFrame;
        function TogFrame(){
            $scope.showPayFastForm = false;
            $scope.showPayFastFrame = false;
            $scope.payFastForm = '';
        }
        ...
        $scope.enqDescSpec = 'enquiries@intellicargoi.com';
        ...
        function SendNewEnq(){
            if($scope.isValidEnq){
                ...
                $http({
                    ...
                    else if (getResponse.data === 'Mail failed'){
                        //console.log(getResponse);
        ...
        $scope.userRegister = function(regType){
            ...
            if(($scope.uname === 'pass') && ($scope.mail === 'pass') && ($scope.pass === 'pass') && ($scope.passCon === 'pass')){
                ...
                $http({
                    ...
                    if(getResponse.data.status === 'Yes'){
                        //console.log('User Register Success');
                        $rootScope.loading = false;
                        //$rootScope.showLogPop = true;
                        ...
                        //alert('The server was unable to connect.\r\nPlease try again later.');
                        //console.log(getResponse.data.status);
                        $rootScope.loading = false;
                    }
                    else {
                        $scope.regMsg = "An unexpected error stopped the registration process.\nPlease try again.";
                        //alert('An unexpected error stopped the registration process.\r\nPlease try again.');
                        //console.log(getResponse.data.status);
                        $rootScope.loading = false;
                    }
                });
